...
def miniball_normalize(v: np.ndarray):
    """warning: no center optim"""
    center = v.mean(0)
    v2 = v - center
    r = np.sqrt((v2**2).sum(1)).max()
    return v2 / r
...
def load_and_sample_shape(model_name: str, src_dir: str, sample_n=1e5, rescale_f=0.9):
...
def tet_circumcenter(verts):
    ...
    denominator = 0.5 / (ba_x * cross_cd_x + ba_y *
    ...
    return np.column_stack((circ_x, circ_y, circ_z))+verts[:, 0]
