...
from networks.sdf_encoders import LocalSDFFE
...
def training_step():
    ...
    for i, batch in enumerate(train_iterator):
        ...
        grid_features = encoder(batch['input_sdf'].to(device))
        if config['vcdec_in_glob']:
            glob_features = F.adaptive_max_pool3d(grid_features, output_size=(1, 1, 1)).squeeze(-1).squeeze(-1).squeeze(-1)

        vc = vcdec(
            batch['input_sparse_xyz'].to(device),
            grid_features[
                batch['input_sparse_sdf_idx'][:, 0], :,
                batch['input_sparse_sdf_idx'][:, 1],
                batch['input_sparse_sdf_idx'][:, 2],
                batch['input_sparse_sdf_idx'][:, 3]
            ],
            g_features=glob_features if config['vcdec_in_glob'] else None
        ) * config['sample_rescale_scale'] / config['grid_size']
        if config['vcdec_vc_tanh']:
            vc *= config['vcdec_vc_tanh_scale']
        vc += batch['input_sparse_xyz'].to(device)
        ...
        for j in range(len(batch['input_sparse_sdf_idx_size'])):
            ...
            loss += voroloss(gt_clouds[j].unsqueeze(0), vc[l:r].unsqueeze(0)).mean()
        ...
        VL.update(1e5 * loss.item(), batch['input_sdf'].shape[0])

        # if (i + 1) % (config['num_workers'] // 2) == 0:
        line = 'Epoch: [{0}][{1}/{2}]'.format(epoch + 1, i + 1, len(train_iterator))
...
def reconstruction_subsampling_step():
    pass
...
if __name__ == '__main__':
    ...
    train_dataset = ABCDataset(
        config['tr_obj_dir'], config['tr_surf_dir'], config['tr_sdf_dir'], val=False,
    ...
    eval_dataset = ABCDataset(
        config['val_obj_dir'], config['val_surf_dir'], config['val_sdf_dir'], val=True,
        input_type=config['input_type'],
        grid_size=config['grid_size'], v_start=config['v_start'], v_end=config['v_end'],
        return_gt_mesh=True,
        n_input_pc=config['n_input_pc'], input_pc_transform=input_pc_transform,
    ...
    encoder = LocalSDFFE(config['in_dim'], config['enc_feat_dim']).to(device)
    vcdec = VCDec(3, config['enc_feat_dim'][-1], config['vcdec_feat_dim'], in_glob=config['vcdec_in_glob'],
    ...
    print('Total number of parameters in encoder: {}'.format(cnt_params(encoder.parameters())))
    print('Total number of parameters in sdfnet: {}'.format(cnt_params(vcdec.parameters())))
    ...
    for epoch in tqdm(range(cur_epoch, config['n_epochs']), initial=cur_epoch, total=config['n_epochs']):
        ...
        tqdm.write('Model saved to: {}'.format(os.path.join(config['path2save'], config['model_name'])))
