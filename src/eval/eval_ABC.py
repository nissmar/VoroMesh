...
grid_size = 32
pred_dir = "/home/../data/nmaruani/ABC_NMC{}/".format(
    grid_size if grid_size == 32 else '')
gt_dir = "/home/../data/nmaruani/ABC/"
...
def get_cd_nc_f1_ecd_ef1(q, name_list):
    ...
    for nid in range(name_num):
        ...
        try:
            ...
            pred_points = pred_points/grid_size-0.5
        ...
        q.put([idx, gt2pred_mean_cd1, gt2pred_mean_cd2, pred2gt_mean_cd1, pred2gt_mean_cd2, cd1, cd2, gt2pred_nc, gt2pred_na, pred2gt_nc, pred2gt_na,
              nc, recall, precision, f1, gt2pred_mean_ecd1, gt2pred_mean_ecd2, pred2gt_mean_ecd1, pred2gt_mean_ecd2, ecd1, ecd2, erecall, eprecision, ef1])
...
if __name__ == '__main__':
    ...
    for idx in range(len(obj_names_old)):
        if not obj_names_old[idx] in nw:
            process_id = true_idx % num_of_process
            gt_obj_name = gt_dir + obj_names_old[idx] + "/model.obj"
            pred_obj_name = pred_dir + "test_" + str(idx) + ".obj"
            list_of_list_of_names[process_id].append(
                [process_id, true_idx, gt_obj_name, pred_obj_name])
            true_idx += 1
        else:
            print(obj_names_old[idx])
    ...
    for i in range(num_of_process):
        ...
        workers.append(
            Process(target=get_cd_nc_f1_ecd_ef1, args=(q, list_of_names)))
    ...
    while True:
        item_flag = True
        try:
            idx, gt2pred_mean_cd1, gt2pred_mean_cd2, pred2gt_mean_cd1, pred2gt_mean_cd2, cd1, cd2, gt2pred_nc, gt2pred_na, pred2gt_nc, pred2gt_na, nc, recall, precision, f1, gt2pred_mean_ecd1, gt2pred_mean_ecd2, pred2gt_mean_ecd1, pred2gt_mean_ecd2, ecd1, ecd2, erecall, eprecision, ef1 = q.get(
                True, 1.0)
        except queue.Empty:
            item_flag = False
    ...
    if counter != obj_names_len:
        print("ERROR: counter!=obj_names_len", counter)
        # exit(-1)

    fout = open("result_100000.txt", 'w')

    valid_ind = (numbers_gt2pred_mean_cd1 != 0)
    print(valid_ind.sum())
    fout.write(
        str(np.mean(numbers_gt2pred_mean_cd1[valid_ind]))+"\t"
        + str(np.mean(numbers_gt2pred_mean_cd2[valid_ind]))+"\t"
        + str(np.mean(numbers_pred2gt_mean_cd1[valid_ind]))+"\t"
        + str(np.mean(numbers_pred2gt_mean_cd2[valid_ind]))+"\t"
        + str(np.mean(numbers_cd1[valid_ind]))+"\t"
        + str(np.mean(numbers_cd2[valid_ind]))+"\t"
        + str(np.mean(numbers_gt2pred_nc[valid_ind]))+"\t"
        + str(np.mean(numbers_pred2gt_nc[valid_ind]))+"\t"
        + str(np.mean(numbers_nc[valid_ind]))+"\t"
        + str(np.mean(numbers_recall[valid_ind]))+"\t"
        + str(np.mean(numbers_precision[valid_ind]))+"\t"
        + str(np.mean(numbers_f1[valid_ind]))+"\t"
        + str(np.mean(numbers_gt2pred_mean_ecd1[valid_ind]))+"\t"
        + str(np.mean(numbers_gt2pred_mean_ecd2[valid_ind]))+"\t"
        + str(np.mean(numbers_pred2gt_mean_ecd1[valid_ind]))+"\t"
        + str(np.mean(numbers_pred2gt_mean_ecd2[valid_ind]))+"\t"
        + str(np.mean(numbers_ecd1[valid_ind]))+"\t"
        + str(np.mean(numbers_ecd2[valid_ind]))+"\t"
        + str(np.mean(numbers_erecall[valid_ind]))+"\t"
        + str(np.mean(numbers_eprecision[valid_ind]))+"\t"
        + str(np.mean(numbers_ef1[valid_ind]))+"\n"
    )

    numbers_gt2pred_na = np.mean(numbers_gt2pred_na, 0)
    numbers_pred2gt_na = np.mean(numbers_pred2gt_na, 0)
    for i in range(90):
        fout.write(str(numbers_gt2pred_na[i]) +
                   "\t"+str(numbers_pred2gt_na[i])+"\n")
